...
export default function StepCamera({ sessionId, jobLabel, onComplete }: Props) {
  ...
  // Only devices with more than one camera can actually switch — most booth
  // kiosk webcams have just one, so the button stays hidden there.
        ...
        {uploading && (
          <div className="cam-upload-overlay" role="alert">
            <div className="cam-upload-card">
              ...
              <span>Please don&apos;t close this page.<br />This can take a little longer on a slow connection.</span>
            </div>
          </div>
        )}
      </div>
      ...
      {!previewUrl ? (
        ...
        <>
          ...
          <button className="kiosk-btn-primary" onClick={submit} disabled={busy}>
            <span>{uploading ? 'Submitting…' : 'Submit'}</span>
